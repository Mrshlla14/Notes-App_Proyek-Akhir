import { getAccessToken } from "../utils/profile.js";
...
const ENDPOINTS = {
  ...
  STORY_DETAILS: (id) => `${BASE_URL}/stories/${id}`,
...
export async function getStoryById(id) {
  ...
  try {
    const fetchResponse = await fetch(ENDPOINTS.STORY_DETAILS(id), {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    ...
    if (json.story) {
      if (
        !json.story.location &&
        (json.story.lat !== undefined || json.story.lon !== undefined)
      ) {
        json.story.location = {
          lat: json.story.lat,
          lon: json.story.lon,
